let hq = {
    ...
    */
    ...
    hero: {
        ...
        stats: function(act, stat, data) {
            if (stat == 'move') {
                act.update({
                    'data.stats.squares_moved': act.data.data.stats.squares_moved + data
                });
                return;
            }
            if (stat == 'gold') {
                act.update({
                    'data.stats.gold_found': act.data.data.stats.gold_found + data
                });
                return;
            }
            if (stat == 'damage') {
                act.update({
                    'data.stats.squares_moved': act.data.data.stats.damage_taken + data
                });
                return;
            }
            if (stat == 'kill') {
                let key = `data.stats.kills.${data.target}`
                let tot = (data.target in act.data.data.stats.kills) ? act.data.data.stats.kills[data.target] + 1 : 1;
                act.update({
                    [key]: tot
                });
                return;
