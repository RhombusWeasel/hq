let hq = {
    ...
    */
    ...
    delay: function(func, data) {
        return setTimeout(func(data), Math.random() * 500)
    },
    sort: {
    ...
    hero: {
        ...
        roll_atk: function(act, amt) {
            let tgt = hq.hero.get_target()
            if (tgt == false) return;
            let result = game.specialDiceRoller.heroQuest.rollFormula(`${amt}h`, `${act.name} attacks!`);
            let hits = result.match(/heroskull\.png/g).length || 0;
            result = `${result.split('<hr>')[0]}`;
            let updates = {
                'data.tmp_atk': 0,
                'data.atk_count': act.data.data.atk_count - 1,
                'data.attack_taken': true,
                'data.stats.attacks_made': act.data.data.stats.attacks_made + 1,
                'data.stats.total_hits': act.data.data.stats.total_hits + hits,
            }
            if(hits > 0) {
                result += `
                    <p>${act.name} lands ${hq.pluralize(hits, 'hit', 'hits')}</p>
                `;
                ...
                result += `
                    <p>${act.name} missed.</p>
                `;
                updates['data.atk_count.stats.total_misses'] = act.data.data.stats.total_misses + 1;
            }
            act.update(updates);
            ChatMessage.create({content: result});
            return hits;
        },
        roll_def: function(act, amt) {
            let tgt = act;
            if (tgt == false) return;
            let def_dice = amt;
    ...
    spells: {
        genie: function(act) {
            //Display option to open any door OR attack any entity with 5 combat dice
            hq.dialog.choice_prompt(
                'Genie',
                () => {
                    hq.chat.send('Genie', `${act.name} casts Genie to open a door.`);
                },
                'Open Door',
                () => {
                    let tgt = hq.hero.get_target();
                    if (tgt) {
                        hq.chat.send('Genie', `${act.name} casts Genie to attack ${tgt.name}.`);
                        hq.hero.roll_atk(act, 5);
                    }else{
                        act.update({
                ...
                `Attack`,
                act.data.data.spells.genie.description
            );
        },
        swift_wind: function(act) {
            let tgt = hq.hero.get_target();
            if (tgt) {
                let t = hq.get_actor(tgt.name);
                t.update({
                    'data.tmp_move': 2,
                });
            }else{
        ...
        tempest: function(act) {},
        heal_body: function(act) {
            let tgt = hq.hero.get_target();
            if (tgt) {
                let t = hq.get_actor(tgt.name);
                hq.hero.body.heal(t, 'Heal Body');
            }else{
                act.update({
        ...
        pass_through_rock: function(act) {},
        rock_skin: function(act) {
            let tgt = hq.hero.get_target();
            if (tgt) {
                let t = hq.get_actor(tgt.name);
                t.update({
                    'data.spell_bonus_def': 2,
                });
            }else{
                act.update({
        ...
        courage: function(act) {
            let tgt = hq.hero.get_target();
            if (tgt) {
                let t = hq.get_actor(tgt.name);
                t.update({
                    'data.spell_bonus_atk': 2,
                });
            }else{
                act.update({
        ...
        veil_of_mist: function(act) {},
        water_of_healing: function(act) {
            let tgt = hq.hero.get_target();
            if (tgt) {
                let t = hq.get_actor(tgt.name);
                hq.hero.body.heal(t, 'Water of Healing');
            }else{
                act.update({
    ...
    dialog: {
        ...
        choice_prompt: function(title, func_1, button_name_1, func_2, button_name_2) {
            let sheet = `
                <h3 class="center typed">${title}</h3>
            `;
            console.log(arguments);
