    ...
    */
        ...
        body: {
            heal: function(act, title) {
                ...
                if (cur < max) {
                    ...
                    if (title == 'Potion of Healing') {
                        updates['data.potions.healing.amount'] = act.data.data.potions.healing.amount - 1
                    }
                    act.update(updates);
                }else{
                    hq.dialog.prompt(source, 'You are already at max health.');
                }
            },
        },
    },
    potions: {
        healing: function(act) {
            hq.char.body.heal(act, 'Potion of Healing');
        },
