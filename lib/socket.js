let commands = {
    ...
    sleep: function(data) {
        let tgt = hq.get_actor(data.target);
        if (tgt.data.data.mind > 0) {
            if (hq.hero.roll_def(tgt, tgt.data.data.mind) > 0) {
                hq.chat.send('Sleep', `${data.attacker} casts sleep on ${tgt.name}...`, `${tgt.name} resists the urge to fall asleep.`);
                return;
            }
        }
        hq.chat.send('Sleep',`${data.attacker} casts sleep on ${tgt.name}...`,  `${tgt.name} falls asleep.`);
        hq.get_token(tgt.name).toggleEffect('icons/svg/sleep.svg', {active: true, overlay: true});
    },
}
