let commands = {
    ...
    draw_treasure: function(data) {
        if (game.user.isGM) {
            ...
            if (draw.type == 'potion') {
                hq.dialog.prompt(draw.name, draw.description);
                act.update({data: {potions: {[draw.key]: {value: act.data.data.potions[draw.key].amount + 1}}}});
            }
            if (draw.type == 'gold') {
                hq.dialog.prompt(draw.name, draw.description);
                let r = new Roll(draw.value).evaluate({async: false});
                act.update({
                    'data.gold': act.data.gold + r._total,
                });
            }
        }
    },
