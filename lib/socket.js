let commands = {
    ...
    attack_roll: function(data) {
        ...
        let def_dice = tgt.data.data.atk + tgt.data.data.tmp_atk + tgt.data.data.spell_bonus_atk
        let die_type = tgt.type == 'monster' ? 'm' : 'h';
        let sheilds  = tgt.type == 'monster' ? 'skullshield' : 'lionshield';
        let result = game.specialDiceRoller.heroQuest.rollFormula(`${def_dice}${die_type}`, `${this.actor.name} attacks!`);
        ...
        if (saves.length >= data.hits) {
            ...
            result += `
                <p>${data.name} deals ${hq.pluralize(wounds, 'wound', 'wounds')} to ${tgt.name}!</p>
            `;
            tgt.update({
